...
export const MainLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <aside
        className={`
          fixed top-0 left-0 z-30 h-full w-64 bg-gradient-to-b from-green-800 to-green-900 text-white transform transition-transform duration-300 ease-in-out
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-green-700">
          <div className="mb-4 p-3 bg-green-700/50 rounded-lg">
            <div className="flex items-center gap-3">
              <UserAvatar
                src={user?.foto_url}
      ...
      {filteredBottomNavItems.length > 0 && (
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 shadow-[0_-2px_10px_rgba(0,0,0,0.1)]">
            ...
            {filteredBottomNavItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-xs font-medium truncate ${isActive ? 'text-green-600 font-semibold' : 'text-gray-500'}`}>
                    {item.label}
                  </span>
                </Link>
              );
