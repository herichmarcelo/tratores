      ...
      <div className="p-4 md:p-6 space-y-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <Settings className="w-7 h-7 text-ff-yellow" />
        ...
        <p className="text-gray-500 dark:text-[#B3B3B3]">Gerencie usuários, fazendas e setores do sistema</p>

        <div className="flex items-center gap-1 bg-gray-100 dark:bg-[#1A1A1A] p-1 rounded-lg w-fit overflow-x-auto">
          {tabs.map((tab) => (
            <button
              key={tab.id}
        ...
        {activeTab === 'usuarios' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="border-none shadow-sm dark:bg-[#14141A]">
              <CardHeader>
                <CardTitle className="text-lg flex items-center gap-2 text-gray-900 dark:text-white">
                  {editingUsuarioId ? (
                    <>
                      <Edit className="w-5 h-5 text-ff-yellow" />
                      Editar Usuário
                    </>
                  ) : (
                    <>
              ...
              <CardContent>
                ...
                >
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-[#B3B3B3] mb-1">E-mail *</label>
                    <Input
                      type="email"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-[#B3B3B3] mb-1">Cargo</label>
                    <Input
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {editingUsuarioId && (
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-[#B3B3B3] mb-1">Status</label>
                        <Select
                  ...
                  <div className="flex gap-3">
                    {editingUsuarioId && (
                      <Button
                        type="button"
                        variant="outline"
                        className="flex-1 border-gray-200 dark:border-[#2A2A2A] dark:text-gray-600 dark:text-white hover:bg-gray-50 dark:hover:bg-[#1A1A1A]"
                    ...
                    <Button
                      type="submit"
                      className="bg-ff-yellow text-black hover:brightness-110 flex-1"
            ...
            <Card className="border-none shadow-sm dark:bg-[#14141A]">
              <CardHeader>
                <CardTitle className="text-lg text-gray-900 dark:text-white">Usuários cadastrados</CardTitle>
              </CardHeader>
              <CardContent className="p-0">
                {usuariosLoading ? (
                  ...
                  <div className="divide-y divide-gray-100 dark:divide-[#2A2A2A] max-h-[480px] overflow-y-auto">
                    {usuarios?.length === 0 ? (
                      <p className="p-6 text-center text-gray-500 dark:text-[#B3B3B3]">Nenhum usuário cadastrado</p>
                    ) : (
                      usuarios?.map((usuario) => (
                        <div
                          key={usuario.id}
                          className={`p-4 flex items-center gap-3 transition-colors ${
                            editingUsuarioId === usuario.id ? 'bg-ff-yellow/10' : 'hover:bg-gray-50 dark:hover:bg-[#1A1A1A]'
                          }`}
                        >
                          ...
                          <div className="flex flex-col items-end gap-1 shrink-0">
                            <Badge className="bg-ff-yellow/20 text-ff-yellow border-ff-yellow/30">
                              {perfilLabels[usuario.perfil]}
                            </Badge>
                            <Badge className={usuario.ativo ? 'bg-ff-green-active/20 text-ff-green-active border-ff-green-active/30' : 'bg-gray-100 dark:bg-[#1A1A1A] text-gray-600 dark:text-[#B3B3B3]'}>
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            className="text-gray-500 dark:text-[#B3B3B3] hover:text-ff-yellow shrink-0"
        ...
        {activeTab === 'fazendas' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="border-none shadow-sm dark:bg-[#14141A]">
              ...
              <CardContent>
                <form onSubmit={handleCreateFazenda} className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-[#B3B3B3] mb-1">Nome da fazenda *</label>
                    <Input
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-[#B3B3B3] mb-1">Razão social</label>
                    <Input
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-[#B3B3B3] mb-1">Inscrição estadual</label>
                      <Input
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-[#B3B3B3] mb-1">CPF do proprietário</label>
                      <Input
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-[#B3B3B3] mb-1">Endereço</label>
                    <Input
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-[#B3B3B3] mb-1">Cidade</label>
                      <Input
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-[#B3B3B3] mb-1">Estado</label>
                      <Input
            ...
            <Card className="border-none shadow-sm dark:bg-[#14141A]">
              <CardHeader>
                <CardTitle className="text-lg text-gray-900 dark:text-white">Fazendas cadastradas</CardTitle>
              </CardHeader>
              <CardContent className="p-0">
                {fazendasLoading ? (
                  <div className="p-8 flex justify-center">
                  ...
                  <div className="divide-y divide-gray-100 dark:divide-[#2A2A2A] max-h-[480px] overflow-y-auto">
                    {fazendas?.length === 0 ? (
                      <p className="p-6 text-center text-gray-500 dark:text-[#B3B3B3]">Nenhuma fazenda cadastrada</p>
                    ) : (
                      fazendas?.map((fazenda) => (
                        <div key={fazenda.id} className="p-4 flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <p className="font-semibold text-gray-900 dark:text-white">{fazenda.nome}</p>
                            {fazenda.razao_social && (
                              <p className="text-sm text-gray-600 dark:text-[#B3B3B3]">{fazenda.razao_social}</p>
                            )}
                            {fazenda.endereco && (
                              <p className="text-sm text-gray-500 dark:text-[#B3B3B3] mt-0.5">{fazenda.endereco}</p>
                            )}
                            ...
                            <div className="flex flex-wrap gap-2 mt-1">
                              {fazenda.inscricao_estadual && (
                                <span className="text-xs text-gray-400 dark:text-gray-500">IE: {fazenda.inscricao_estadual}</span>
                              )}
                              {fazenda.cpf_proprietario && (
                                <span className="text-xs text-gray-400 dark:text-gray-500">CPF: {fazenda.cpf_proprietario}</span>
                              )}
                            </div>
                          </div>
                          <Badge className={`shrink-0 ${fazenda.ativo ? 'bg-ff-green-active/20 text-ff-green-active border-ff-green-active/30' : 'bg-gray-100 dark:bg-[#1A1A1A] text-gray-600 dark:text-[#B3B3B3]'}`}>
        ...
        {activeTab === 'setores' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="border-none shadow-sm dark:bg-[#14141A]">
              <CardHeader>
                <CardTitle className="text-lg text-gray-900 dark:text-white">Setores cadastrados</CardTitle>
              </CardHeader>
              <CardContent className="p-0">
                {setoresLoading ? (
                  <div className="p-8 flex justify-center">
                  ...
                  <div className="divide-y divide-gray-100 dark:divide-[#2A2A2A] max-h-[480px] overflow-y-auto">
                    {setores?.length === 0 ? (
                      <p className="p-6 text-center text-gray-500 dark:text-[#B3B3B3]">Nenhum setor cadastrado</p>
                    ) : (
                      setores?.map((setor) => (
                        <div key={setor.id} className="p-4 flex items-center justify-between gap-3">
                          <div>
                            <p className="font-semibold text-gray-900 dark:text-white">{setor.nome}</p>
                            {setor.fazenda && (
                              <p className="text-sm text-gray-500 dark:text-[#B3B3B3]">{setor.fazenda.nome}</p>
                            )}
                          </div>
